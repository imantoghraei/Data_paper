...
contains
  ...
  SUBROUTINE FLOTT_GWD_rando(DTIME, pp, tt, uu, vv, prec, zustr, zvstr, d_u, &
    ...
    !END ONLINE
    ! OFFLINE
      RUWMAX= 0.24    ! Launched flux
    ! RUWMAX= 15.00    ! Launched value to reache 10mPa at 100hPa the 11/2005
    ! RUWMAX= 0.200    ! Launched value to reache 1mPa at 100hPa the 11/2005
    ! RUWMAX= 0.130    ! Launched value to reache 1mPa at 100hPa the 05/1993
    ! RUWMAX= 6.5    ! Launched value to reache 10mPa at 100hPa the 05/1993
      SAT=0.60     ! Saturation parameter
    ! END OFFLINE
 ...
 ! Characteristic depth of the source
    ...
    DO LL = 1, KLEV
       ...
       IF (PH(10398, LL) / PH(10398, 1) > XLAUNCH) LAUNCH = LL !IT: KLON / 2 is changed to a grid point in the oceans (ipix=10398), since we are using healpix grids
    ENDDO
    DO LL = 1, KLEV
       !IF (PH(KLON / 2, LL) / PH(KLON / 2, 1) > XTROP) LTROP = LL
       IF (PH(10398, LL) / PH(10398, 1) > XTROP) LTROP = LL !IT: KLON / 2 is changed to a grid point in the oceans (ipix=10398), since we are using healpix grids
    ENDDO
...
!   ENDDO
    ...
    ZVSTR = 0.
    DO LL = 1, KLEV
       ZVSTR = ZVSTR + D_U(:, LL) / RG * (PH(:, LL + 1) - PH(:, LL))/DTIME
    ENDDO

!   PRINT *,'PH in GWD_rando:'
!   DO LL=1,KLEV; PRINT *,LL,PH(KLON/2,LL);ENDDO
