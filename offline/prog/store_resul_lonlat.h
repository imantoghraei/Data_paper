...
!  3-D Variables
      ...
      do ii=1,iim;do jj=1,jjm;ij=ii+(jj-1)*iim
      var3d(ii,jj,:)=zustror_z_lonlat(ij,:);enddo;enddo
      status = nf90_put_var(ncio,rvarid,var3d,start=(/1,1,1,iter/),&
                            count=(/iim,jjm,llm,1/))

      status = nf90_inq_varid(ncio,"u_prstress",rvarid)
      do ii=1,iim;do jj=1,jjm;ij=ii+(jj-1)*iim
      var3d(ii,jj,:)=zustrpr_z_lonlat(ij,:);enddo;enddo
      status = nf90_put_var(ncio,rvarid,var3d,start=(/1,1,1,iter/),&
                            count=(/iim,jjm,llm,1/))

      status = nf90_inq_varid(ncio,"u_frstress",rvarid)
      do ii=1,iim;do jj=1,jjm;ij=ii+(jj-1)*iim
      var3d(ii,jj,:)=zustrfr_z_lonlat(ij,:);enddo;enddo
      status = nf90_put_var(ncio,rvarid,var3d,start=(/1,1,1,iter/),&
                            count=(/iim,jjm,llm,1/))

      status = nf90_inq_varid(ncio,"u_stress_orpr",rvarid)
      do ii=1,iim;do jj=1,jjm;ij=ii+(jj-1)*iim
      var3d(ii,jj,:)=zustr_z_lonlat(ij,:);enddo;enddo
      status = nf90_put_var(ncio,rvarid,var3d,start=(/1,1,1,iter/),&
                            count=(/iim,jjm,llm,1/))

      status = nf90_inq_varid(ncio,"u_stress_orprfr",rvarid)
      do ii=1,iim;do jj=1,jjm;ij=ii+(jj-1)*iim
      var3d(ii,jj,:)=zustr_z2_lonlat(ij,:);enddo;enddo
      status = nf90_put_var(ncio,rvarid,var3d,start=(/1,1,1,iter/),&
                            count=(/iim,jjm,llm,1/))
